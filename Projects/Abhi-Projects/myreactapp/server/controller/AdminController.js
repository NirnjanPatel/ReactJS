...
class AdminController {
    ...
    addProperty(title, subcatname, description, piconname) {
        return new Promise((resolve, reject) => {
            var pDetails = { _id: Date.now(), "title": title, "subcatname": subcatname, "description": description, "piconname": piconname };
            AdminModel.addProperty(pDetails).then((result) => {
                resolve(result);
            }).catch((err) => {
                reject(err);
            });
